...
struct PosicionListaCRep {
  ...
  struct PosicionListaCRep *anterior;
};
...
void insertaLC(ListaC codigo, PosicionListaC p, Operacion o) {
  ...
  nuevo->sig = p->sig;
  nuevo->anterior = p;
  if (p->sig != NULL) p->sig->anterior = nuevo;  
  p->sig = nuevo;
...
//Posible error
void concatenaPosLC(ListaC codigo1, ListaC codigo2, PosicionListaC pos){
  NodoPtr aux = codigo2->cabecera;
  while (aux->sig != NULL) {
    insertaLC(codigo1,pos,aux->sig->dato);
    aux = aux->sig;
    pos = pos->sig;
  }
}
